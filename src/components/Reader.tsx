...
import { copyText, getProjectFile, openExternalLink, readNativeAsset, resolveRelativePath } from "../file-system";
  ...
  return (
    <main className="reader-shell">
      ...
      <div ref={gridRef} className={`reader-grid mode-${mode}`} style={mode === "split" ? { gridTemplateColumns: `minmax(280px, ${splitRatio}fr) 5px minmax(280px, ${100 - splitRatio}fr) 190px` } : undefined}>
        {(mode === "source" || mode === "split") && <SourceView key={`source-${doc.id}`} source={source} onChange={onSourceChange} theme={theme} previewRef={previewRef} searchRequest={sourceSearchRequest} onActivate={() => setSearchTarget("source")} />}
        ...
        {(mode === "preview" || mode === "split") && <Preview key={`preview-${doc.id}`} ref={previewRef} html={previewSearch.html} doc={doc} project={project} onOpenDoc={onOpenDoc} onActivate={() => setSearchTarget("preview")} onOpenImage={(src, alt) => { setImageViewer({ src, alt }); setImageScale(1); }} />}
